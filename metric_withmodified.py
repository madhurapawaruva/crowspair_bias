...
def get_span(seq1, seq2):
    ...
    for op in matcher.get_opcodes():
        ...
        if op[0] == 'equal':
            ...
            non_template1 += [x for x in range(op[1], op[2], 1)]
            non_template2 += [x for x in range(op[3], op[4], 1)]
    # print("non template 1: {}, non template 2 :{}".format(non_template1,non_template2))     
    return template1, template2, non_template1, non_template2
...
def mask_unigram(data, lm, n=1):
    ...
    template1, template2, non_template1, non_template2 = get_span(sent1_token_ids[0], sent2_token_ids[0])
    # modified_token = 
    ...
    # get probability of mmodified token

    mod_score1, mod_score2 = 0, 0
    if non_template1 and non_template2:
        sent1_masked_token_ids = sent1_token_ids.clone().detach()
        sent2_masked_token_ids = sent2_token_ids.clone().detach()
        sent1_masked_token_ids[0][non_template1[0]] = mask_id
        sent2_masked_token_ids[0][non_template2[0]] = mask_id

        mod_score1 = get_log_prob_unigram(sent1_masked_token_ids, sent1_token_ids, non_template1[0], lm)
        mod_score2 = get_log_prob_unigram(sent2_masked_token_ids, sent2_token_ids, non_template2[0], lm)
        mod_score1, mod_score2 = mod_score1.item(), mod_score2.item()


    score = {}
    # average over iterations
    sent1_score = sent1_log_probs + lambda_*mod_score1
    sent2_score = sent2_log_probs + lambda_*mod_score2
    score["sent1_score"] = sent1_score
    score["sent2_score"] = sent2_score
